...
// Owner numbers
const ownerNumber = ["923094676374"];
...
if (!fs.existsSync(CREDS)) {
  ...
  console.log("♻️ MOEED-MD session restored successfully");
}
    ...
    res.send(`
      ...
      <body>
        <div class="container">
          ...
          <p>Owner: ${config.OWNER_NAME || 'MOEED'}</p>
        </div>
      </body>
      </html>
    `);
...
async function connectToWA() {
  ...
  sock.ev.on('connection.update', (update) => {
    ...
    if (connection === 'close') {
      ...
      console.log("✅ MOEED-MD Bot connected to WhatsApp!");
      ...
      try {
        ...
        sock.sendMessage(ownerNumber[0] + '923094676374@s.whatsapp.net', {
          image: { url: imageUrl },
          caption: aliveMsg
        }).catch(() => {
